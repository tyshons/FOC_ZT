...
ESO使用独立的兼容扩展帧，不改变原有速度、位置、PID、普通遥测或扫频协议：

- `0x05/0x1B`，7字节：查询ESO状态；
- `0x05/0x1B`，20字节：设置带宽、补偿增益和独立电流限幅；
- `0x05/0x1C`，67字节：回报当前模式、ESO启用/限幅状态、运行参数、原始与实际补偿电流、合成电流、`z1/z2`、速度观测误差、剩余扰动转矩和累计限幅比例所需计数。

上位机实验面板打开时以10 Hz查询ESO状态，并将最近一次回报与20 Hz主遥测按时间对齐写入CSV。默认带宽30 rad/s、补偿增益0.2、独立限幅0.3 A；下位机拒绝超出安全范围或包含非有限数的配置。

### 6.7 学习前馈控制与状态

学习前馈沿用实验配置帧 `0x05/0x15`：标志位0用于开始或继续学习，标志位1用于暂停并清空RAM中的学习表。暂停学习只停止新一圈数据的采集和更新；在模式4或5内，已经发布的学习表仍继续输出。切换到基线模式会停用学习表，但不会清除RAM内容；复位或重新上电会清除学习结果。

新增只读状态接口，不改变FOC驱动、电流环、速度环或编码器采样：

- `0x05/0x1D`，7字节：查询学习前馈状态；
- `0x05/0x1D`，51字节：回报当前实验模式、学习请求和运行状态、学习策略、活动表、已选阶次数、位置槽、总圈数、有效覆盖圈数、更新次数、丢弃圈数、最近覆盖率、相关度均值、学习表RMS/峰值电流和当前学习前馈电流。

上位机实验面板打开时以2 Hz查询学习状态，并提供“清空学习表”“开始/继续学习”“暂停学习”和“查询学习状态”四个入口。开始学习前会检查下位机已经处于模式4或5、方位轴已使能、没有FOC故障，并要求最近速度均值为4.75～5.25 RPM且全部样本位于3.5～6.5 RPM。上述状态量会与20 Hz主遥测按时间对齐写入导出的CSV。
